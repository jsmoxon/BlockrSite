...
def create_total_commit_list(username, repo_list, commit_goal):
    ...
    total_commit_list = []
    for r in repo_list:
        total_commit_list.append(check_commit_list(username,r, commit_goal))
...
def check_for_recent_commits(time, total_commit_list, commit_goal):
    """
...
def what_should_flag_be(time, username, commit_goal):
    if check_for_recent_commits(time, create_total_commit_list(username, create_repo_list(username), commit_goal), commit_goal) > commit_goal:
        return True
    else:
        return False
